...
<%
	...
	try {
		...
		pstmt = conn.prepareStatement("select cno from tc where tno=? and semester=?;");
		pstmt.setInt(1, tno);
...
<% 
		while(rs.next()) {
			try {
				pstmt_cno = conn.prepareStatement("select name, credit, place_time from course where cno=?;");
