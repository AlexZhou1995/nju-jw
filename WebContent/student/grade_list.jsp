...
<%
	...
	try {
		...
		pstmt = conn.prepareStatement("select cno, type, score from sc where sno=? and semester=? and score is not null;");
...
<% 
		while(rs.next()) {
			try {
				pstmt_cno = conn.prepareStatement("select name from course where cno=?;");
